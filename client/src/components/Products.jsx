...
const Products = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      await fetch("http://localhost:3001/products/getallproducts")
  ...
  const Submithandle=async(e)=>{
    ...
    if(product.name.length>0){
     const result=await axios.post('http://localhost:3001/products/add',product);
     setAlert(false)
  ...
  const update=async(id)=>{
     const result=await axios.post("http://localhost:3001/products/deleteProduct",{id})
      ...
      <table class="table table-success table-striped">
        <thead>
          <tr class="table-success">
            ...
            <th className="table-success">Deletev </th>
          </tr>
        </thead>
        <tbody>
